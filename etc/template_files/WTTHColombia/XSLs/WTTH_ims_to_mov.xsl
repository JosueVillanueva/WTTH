   ...
   <xsl:output indent="yes" xalan:indent-amount="2" xmlns:xalan="http://xml.apache.org/xslt"/>
    ...
    <xsl:template match="/">
    	<localRoutes xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	    	<xsl:for-each select="$movil">
	    	
	    		<xsl:variable name="count_fixed" select="position()"></xsl:variable>
	    		<route>
	    			<user type="E164"><xsl:value-of select="." /></user> 
	    			<next type="regex">!sip:<xsl:value-of select="$fixed[$count_fixed]"/>@mgcfims!</next>
	    		</route>
			</xsl:for-each>
    	</localRoutes>
    </xsl:template>
